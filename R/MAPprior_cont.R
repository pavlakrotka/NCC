...
#' @param prior_prec_mu Dispersion parameter of the normal prior, the prior for the control mean. Default=0.001.
...
MAPprior_cont <- function(data,
  ...
  if (check) {
    ...
    if(!is.numeric(prior_prec_mu) | length(prior_prec_mu)!=1){
      stop("The dispersion parameter of the normal prior, the prior for the control mean, (`prior_prec_mu`) must be one number!")
    }
